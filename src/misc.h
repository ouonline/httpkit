...
static inline void set_content_len(const char* base, const struct http_kv_ol_list* hdr_list,
                                   unsigned long* content_len) {
    int ret = http_kv_ol_list_for_each(hdr_list, base, content_len, __check_content_length);
    if (ret == 0) {
        *content_len = 0;
    }
}
